...
Since these backgrounds are not expected to occur outside the GdLS, we must clamp the vertex cut parameters at the limits defined above. The radial factor is then
\[
  f_r = \left(\frac{\min(r_{\mathrm{max}}, r_{\mathrm{max}}^{\mathrm{nom}})}{r_{\mathrm{max}}^{\mathrm{nom}}}\right)^2 - \left(\frac{r_{\mathrm{min}}}{r_{\mathrm{max}}^{\mathrm{nom}}} \right)^2.
\]
...
\[
  f_z = \frac{\max(z_{\mathrm{min}}, z_{\mathrm{min}}^{\mathrm{nom}}) - \min(z_{\mathrm{max}}, z_{\mathrm{max}}^{\mathrm{nom}})}{z_{\mathrm{max}}^{\mathrm{nom}} - z_{\mathrm{min}}^{\mathrm{nom}}}.
\]

Each of these backgrounds is then adjusted according to
\[
