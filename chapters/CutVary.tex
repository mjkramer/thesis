...
\begin{figure}[ht]
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.5]{CutVary/ShowerVeto/veto_eff.eh3_ad1.8ad.pdf}%
  \end{minipage}%
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.5]{CutVary/ShowerVeto/veto_eff.eh3_ad1.8ad_toymc.pdf}%
  \end{minipage}%
...
\autoref{fig:cutVaryVetoEffDataResults} shows the results of running the IBD selection and fitter on the P17B dataset, on the same grid of veto parameters as used in the toy MC study. In this case we see a greater degree of variation, which is unsurprising given that, in data, modifying the shower veto will produce statistical fluctuations, while this effect is abasent for the toy MC. Nonetheless, in a large region of parameter space, including both the LBNL and IHEP vetoes, the fit is acceptably stable. Significant deviations are observed only on the left edge, where the loose definition of a shower muon results in an unreasonably low veto efficiency.
...
In the reconstructed energy spectrum from nGd captures, there is a long and tall tail below the nominal cut of 6~MeV (\autoref{fig:cutVaryDelCutDelSpec}). This tail is populated by events in which one or more gammas ``leak'' beyond the active volume of the AD, leaving some of the initial energy unreconstructed. Fitting the delayed spectrum to a double Crystal Ball function (\autoref{eq:cbfunction}) suggests that some 10\% of nGd captures fall below the 6~MeV cut. Due to this potential of gaining O(10\%) in signal statistics, it is worthwhile to explore the variation of the 6~MeV cut. Here we study modified delayed cuts ranging from 4 to 7~MeV.
...
The AmC background possesses a different delayed energy spectrum (domninated by neutron captures in the AD's stainless steel vessel); unforunately, none of the available internal studies of the AmC background provide the delayed spectrum, neither from simulations nor from the runs with the strong AmC source. Due to the small size of this background, we simply ignore any potential dependence of its rate on the delayed cut. We will later see that, at least in the neighborhood of a 6~MeV delayed cut, the oscillation analysis is remarkably stable, in spite of this simplification.

Although these correlated backgrounds do increase as the delayed cut is loosened, the effect on the uncertainty is outweighed by the increase in signal statistics. Letting $k$ denote this scaling factor (i.e.\ the ratio of the new delayed efficiency to the old one):
\begin{align}
...
Unfortunately, the accidental background is not as well-behaved. As shown in \autoref{fig:cutVaryDelCutNcapExample}, the spectrum of delayed-like singles rises much more steeply below 5  MeV compared to the neutron capture spectrum, implying that the accidental background increases faster (percentage-wise) than the IBD rate (as confirmed in \autoref{fig:cutVaryDelCutAccBkg}). When this effect is large enough, the overall uncertainty on the background-subtracted IBD rate no longer decreases (as illustrated in \autoref{eq:cutVaryDelayedNcorrSigma}); instead, it \emph{increases}. In principle, then, it is the accidental background that serves to penalize overly loose definitions of the delayed energy cut.\footnote{In saying this, we are ignoring the possibility of new, unaccounted-for \emph{correlated} backgrounds (or increased AmC backgrounds) appearing when we loosen the delayed cut. As we discuss later, this is in fact a likely issue.}
...
For fits to actual data, the accidentals background is calculated by the IBD selector as described in \autoref{sec:accratecalc}. This calculation is valid for any delayed energy cut, so no further correction is necessary.
...
\begin{figure}[ht]
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/s2t_mid_toymc.png}%
  \end{minipage}%
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/dm2_mid_toymc.png}%
  \end{minipage}%
...
\begin{figure}[ht]
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.47]{CutVary/DelayedCut/s2t_mid_methcomp.png}%
  \end{minipage}%
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.47]{CutVary/DelayedCut/dm2_mid_methcomp.png}%
  \end{minipage}%
...
\begin{figure}[ht]
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/unc_s2t_toyVsData.png}%
  \end{minipage}%
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/unc_dm2_toyVsData.png}%
  \end{minipage}%
...
\begin{figure}[ht]
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/BCW/s2t_mid.png}%
  \end{minipage}%
  \begin{minipage}{0.5\linewidth}%
    \includegraphics[scale=0.48]{CutVary/DelayedCut/BCW/dm2_mid.png}%
  \end{minipage}%
...
\begin{figure}[h]
  \includegraphics[scale=0.6]{CutVary/PromptCut/BkgSpec/bkgspec_Li9.png}
  \caption{Prompt spectrum of the $^9$Li background. Generated from the input file used by the fitter.}
  \label{fig:bkgspec_Li9}
\end{figure}

\begin{figure}[h]
  \includegraphics[scale=0.6]{CutVary/PromptCut/BkgSpec/bkgspec_fn.png}
...
\begin{figure}[h]
  \includegraphics[scale=0.6]{CutVary/PromptCut/BkgSpec/bkgspec_AmC.png}
  \caption{Prompt spectrum of the AmC background. Generated from the input file used by the fitter.}
  \label{fig:bkgspec_AmC}
\end{figure}

\begin{figure}[h]
  \includegraphics[scale=0.6]{CutVary/PromptCut/BkgSpec/bkgspec_alphaN.png}
...
\begin{figure}[ht]
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/PromptCut/s2t_mid.pdf}%
  \end{minipage}%
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/PromptCut/dm2_mid.pdf}%
  \end{minipage}%
...
\begin{figure}[ht]
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/PromptCut/unc_s2t.pdf}%
  \end{minipage}%
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/PromptCut/unc_dm2.pdf}%
  \end{minipage}%
...
\begin{figure}[ht]
  \includegraphics[scale=0.35]{CutVary/VertexCut/both_labeled.png}%
  \caption{Results of oscillation fit when applying a vertex cut to the otherwise-nominal selection}
  \label{fig:cutVaryVtxCutJustVertex}
\end{figure}
...
\begin{figure}[ht]
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/VertexCut/s2t_best.png}%
  \end{minipage}%
  \begin{minipage}{0.47\linewidth}%
    \includegraphics[scale=0.48]{CutVary/VertexCut/dm2_best.png}%
  \end{minipage}%
...
\begin{figure}[h]
  \includegraphics[scale=0.5]{CutVary/JointVary/s2t_best_scatter.png}
