...
\[ A^\mathrm{snf}_c =  \frac{\frac{1}{6}\sum_{c'} \int F^\mathrm{ne}_{c'}(E)\,\sigma(E)\, dE}{\int S^\mathrm{snf}_c(E)\,\sigma(E)\, dE} \; R^\mathrm{snf},
\]
...
It is this $F_c(E)$ that is fed into the fitter and toy MC framework. Again, its units are 10$^{18}$~$\widebar\nu_e$~MeV$^{-1}$~s$^{-1}$.
