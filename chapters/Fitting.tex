...
\end{equation}
where the three terms correspond to detector nonuniformity, photoelectron statistics, and noise, respectively. In fig XXX we plot this function, along with its uncertainty bands (discussed later). The coefficients were determined by performing a fit to the gamma-ray peaks from various calibration sources and neutron captures, as well as to the alpha-particle peaks from radioactivity, as shown in \autoref{fig:resolutionFit}. This model does not account for the intrinsic energy spread of the IBD positron due to its angular distribution. We proceed to justify this omission, first heuristically and then quantitatively.
