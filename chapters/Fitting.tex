...
different approaches, theoretically equivalent but implemented very differently.
These are the method of pull terms, and the covariance matrix approach. In this
analysis, we use the latter, but both will be briefly described in what follows.
...
variation required to increase the reduced $\chi^2$ by one unit (minimizing over
the pull terms at every step). Correlations between spectral energy bins are
handled implicitly; the information is encoded in the manner in which different
bins move together when pull terms are varied.
...
``toy'' MC, described in the next section, was developed for this purpose.
...
\begin{flalign*}
  ...
  \text{ is the weekly average energy per fission, over all isotopes, in MeV, and} \\
...
simply as
\[ F_c(E) = F^\mathrm{ne}_c(E) + A^\mathrm{snf}_c \, S^\mathrm{snf}_c(E). \] It
is this $F_c(E)$ that is fed into the fitter and toy MC framework. Again, its
units are 10$^{18}$~$\widebar\nu_e$~MeV$^{-1}$~s$^{-1}$.
