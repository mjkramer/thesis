...
simply as \[ F_c(E) = F^\mathrm{ne}_c(E) + A^\mathrm{snf}_c \,
  S^\mathrm{snf}_c(E). \] It is this $F_c(E)$ that is fed into the fitter and
toy MC framework. Again, its units are
10$^{18}$~$\widebar\nu_e$~MeV$^{-1}$~s$^{-1}$.
