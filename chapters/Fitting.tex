...
\begin{flalign*}
  \text{where } S_{ie} & \text{ is the theoretical spectrum in $\overline\nu_e$~MeV$^{-1}$~fission$^{-1}$,} & \\
  T_{w} & \text{ is the weekly livetime, in days,} \\
  \widebar P & \text{ is the nominal core power, 2895~MW, } \\
  P_w & \text{ is the actual core power, as a fraction of $\overline P$,} \\
  q & \text{ is 1.602$\times10^{-19}$ J/eV,} \\
  f_{iw} & \text{ is the weekly fission fraction of isotope $i$, } \\
  E_i & \text{ is the average energy per fission of isotope $i$} \\
  \widebar E_w & \equiv \sum_i f_{iw} E_i \text{ is the weekly average energy per fission, over all isotopes,} \\
