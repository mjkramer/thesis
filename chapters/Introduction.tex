...
\begin{equation}
  \begin{aligned}
    ...
    \ket{\nu(x, t)} &= \exp \left( -iEt + ip_ix \right) U^*_{\alpha i}\ket{\nu_i} \\
    &\approx \exp \left( -iEt + i(E - \frac{m_i^2}{2E}x ) \right) U^*_{\alpha i}\ket{\nu_i} \\
    &= \exp \left( -iE(t - x) + i\frac{m_i^2}{2E}x \right) U^*_{\alpha i}\ket{\nu_i}.
  \end{aligned}
\end{equation}
We are interested in oscillations over a distance $L$, so we must take $\ket{\nu(L, t)}$. But what is $t$? The answer is that it doesn't matter: $\exp(-iEt)$ is a common phase factor for all three plane-wave components, so it will have no effect on the oscillation probability. We are thus free to choose
\begin{equation}
  t = L.
\end{equation}
With this choice of $t$, and letting
\begin{equation}
  \ket{\nu(x)} \equiv \ket{\nu(x, x)},
\end{equation}
\autoref{eq:nuStateSpacetimeDep} simplifies to
\begin{equation}
  \ket{\nu(L)} \approx \exp \left( i \frac{m_i^2 L}{2E} \right) U^*_{\alpha i} \ket{\nu_i}.
\end{equation}
