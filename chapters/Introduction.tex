...
\end{equation}
For all intents and purposes, this approximation is exact at Daya Bay. Hence Eqs. (\ref{eq:dmPhysical}) and (\ref{eq:hierarchy}) can be used to relate Daya Bay's measured \(\Delta m^2_{ee}\), as operationally defined by (\ref{eq:survProbDybEE}), to the physical mass splittings:
\begin{align*}
  \Delta m^2_{31} = \Delta m^2_{ee} + \sin^2 \theta_{12} \Delta m^2_{21}, \qquad
  & \Delta m^2_{32} = \Delta m^2_{ee} - \cos^2 \theta_{12} \Delta m^2_{21} \qquad
  & \text{(NH)}\\
  \Delta m^2_{31} = \Delta m^2_{ee} - \sin^2 \theta_{12} \Delta m^2_{21}, \qquad
  & \Delta m^2_{32} = \Delta m^2_{ee} + \cos^2 \theta_{12} \Delta m^2_{21} \qquad
  & \text{(IH)}
