...
\item $\mathbf{E_{rec}}$\textbf{:} The final \emph{reconstructed energy}, obtained from $E_{\mathrm{cor}}$ by applying an additional correction for the time-dependence of the geometric nonuniformity\footnote{Caused by, e.g., the gradual increase in the attenuation length of the scintillator.} This is stored as the output of the AdSimple reconstruction. Two events which occur at different positions/times, but which are otherwise identical, will produce the same $E_{\mathrm{rec}}$. The energy scale is defined such that $E_{\mathrm{rec}} = \SI{7.94}{MeV}$ for spallation neutrons.
