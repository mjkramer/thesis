...
\begin{equation}
  ...
  \label{eq:reconBirks}
\end{equation}
where $Q$ is the emitted light, $dE/dx$ is the density of \emph{ionization} energy deposition, and the scintillator-specific value $k_B$ is known as Birks' constant.

If all energy is deposited as ionization energy, and the $dE/dx$ profile is known across the particle's range, then \eqref{eq:reconBirks} can simply be integrated in conjunction with the $dE/dx$ profile. Unfortunately, the reality at Daya Bay is far more complex. Electrons, for instance, can undergo hard scatters with each other, or produce gammas via bremmstrahlung. And gammas, in turn, do not themselves ionize, but they do produce and scatter electrons and positrons. A positron, finally, can be regarded as an electron during the deposition of its kinetic energy, and then as two 0.511~MeV gammas afterward. That is,
\begin{equation*}
  E_{\mathrm{vis},e^+}(E_{\mathrm{kin}}) = E_{\mathrm{vis},e^-}(E_{\mathrm{kin}}) + 2 \times E_{\mathrm{vis},\gamma}(\SI{0.511}{MeV}).
\end{equation*}
Given all of this complexity, a Geant4-based MC simulation was used to compute the aggregate effect of all these microscopic interactions, as a function of Birks' constant $k_B$.

This simulation also accounts for Cerenkov radiation, i.e., the optical equivalent of a supersonic shock wave, produced when a particle is moving faster than the phase velocity of light in a medium. Although Cerenkov emission is the sole source of light in the Daya Bay water pools, it plays a subpercent role in the ADs. Geant4's optical models are able to accurately capture the shape of the relationship between Cerenkov emission and particle energy, but the scale of the Cerenkov nonlinearity, relative to that from quenching, must be determined by fitting to measured data.
