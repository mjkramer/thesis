...
Due to this last property, the expected timewalk-corrected time for a ``large'' pulse, with TDC count $N$, is simply $-N/f_{mathrm{TDC}}$, where $f_{\mathrm{TDC}} = 1/(1.625\;\text{ns})$. As shown in Fig.~XXX, typical physical hits have TDC counts of 950--1000, corresponding (respectively) to times of -1545 to -1625~ns. The details of selecting physical hits, including the exact range of times used, are discussed in \autoref{sec:reconEnergyChage}.
