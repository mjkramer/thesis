...
\begin{table}[h]
  \begin{tabular}{lccccc}
    ...
    \bottomrule
  \end{tabular}
  \caption{Final estimates of the daily $^9$Li rate (per AD) in each hall, according to \autoref{eq:bkgLi9DailyRate}. The uncertainties of $\epsilon_\mu$, $\epsilon_m$, and $T$ (and consequently $N^{\mathrm{eff}}_{\mathrm{det}}$) are negligible. Note that, since the efficiencies of the muon veto and multiplicity cut (in the $^9$Li+$^8$He selection) were divided out in \autoref{eq:bkgLi9DailyRate}, we must multiply these rates by the corresponding efficiencies of our IBD selection in order to obtain the predicted number of $^9$Li+$^8$He events in our raw IBD sample.}
...
\begin{figure}[ht]
  % misc_ana/MiscPlots/plot_li9_linreg.jl
  % plotpoints_nominal.([1, 2, 3])
