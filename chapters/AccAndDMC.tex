...
Sec.~\ref{sec:avoidmuoncorr}). Likewise, $t$ can be chosen arbitrarily, provided
...
In the selection criteria described in Sec.~\ref{sec:singsel}, the final one
...
As a final verification of Eq.~\ref{eq:effdecoup}, let us divide rejected events
...
\begin{align*}
  \etot &= 1 - \Pa - \Pb - \Pab \\
        ...
        &= \emu \eisol
\end{align*}
Clearly, Eq.~\ref{eq:effdecoup} indeed applies. The muon veto efficiency is
...
= w$). As shown in Fig.~\ref{fig:lambertW}, the $W$ function has two branches.
...
\end{equation}
Finally, inserting this into Eq.~\ref{eq:ident1}, we obtain the isolation cut
efficiency:
\begin{equation}
