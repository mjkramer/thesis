...
In \autoref{eq:fitResults}, we gave the best-fit oscillation parameters obtained when using the nominal IBD selection cuts. These values form the basis of our final result. However, in \autoref{sec:cutVaryDelCutResults}, we found that a $\sim0.2\sigma$ bias in $\SinSq$ (but not $\Dmsqee$) may arise from the nominal assumption that all ADs share the same efficiency of the 6~MeV delayed cut. Using the fitter's reported uncertainty of 0.0029 on $\SinSq$, we take 20\% of this (0.00058), and add it in quadrature to 0.0029 to obtain the total uncertainty (now negligibly larger) on $\SinSq$. Our final result for the oscillation parameters is then (\autoref{eq:fitResults}):
\begin{equation}
  \SinSq = 0.0853 \pm 0.0030 \qquad \Dmsqee = 2.47\times10^{-3} \pm 7.2\times10^{-5}
\end{equation}
